...
G_x2 = signal.correlate2d(im, S_x, mode='same')
G_y2 = signal.correlate2d(im, S_y, mode='same')

magnitude = np.hypot(G_x, G_y)/1081 #max value is 1081, we normalized
v = (magnitude*255)

v2 = (np.hypot(G_x2, G_y2)/1081)*255
...
plt.title('Convolve'), plt.xticks([]), plt.yticks([])
plt.subplot(122),plt.imshow(out2)
plt.title('Correlate'), plt.xticks([]), plt.yticks([])

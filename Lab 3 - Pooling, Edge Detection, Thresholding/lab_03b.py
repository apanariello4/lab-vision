...
G_x = signal.convolve2d(im, S_x)
G_y = signal.convolve2d(im, S_y)

magnitude = np.hypot(G_x, G_y)/1081 #max value is 1081, we normalized
v = (magnitude*255).astype(np.uint8)

theta = np.arctan2(G_x, G_y) #this is [-pi,pi], we need to normalize to [0,179]
h = theta + np.pi
h /= (2*np.pi)
h *= 180
